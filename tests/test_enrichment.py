...
from techno_scan.enrichment import (
...
class TestGetCachedArtist:
    @patch("techno_scan.enrichment.store")
    ...
    @patch("techno_scan.enrichment.is_following", return_value=False)
    @patch("techno_scan.enrichment.store")
    ...
    @patch("techno_scan.enrichment.CACHE_TTL_DAYS", 30)
    @patch("techno_scan.enrichment.is_following", return_value=False)
    @patch("techno_scan.enrichment.store")
    ...
    @patch("techno_scan.enrichment.CACHE_TTL_FOLLOWING_DAYS", 7)
    @patch("techno_scan.enrichment.is_following", return_value=True)
    @patch("techno_scan.enrichment.store")
...
class TestIsCacheStale:
    ...
    @patch("techno_scan.enrichment.CACHE_STALE_DAYS", 14)
    @patch("techno_scan.enrichment.store")
...
class TestGetArtistInfoByRaId:
    @patch("techno_scan.enrichment.save_cached_artist")
    @patch("techno_scan.enrichment.check_rising")
    @patch("techno_scan.enrichment.populate_bandcamp_info", side_effect=lambda x: x)
    @patch("techno_scan.enrichment.populate_discogs_info", side_effect=lambda x: x)
    @patch("techno_scan.enrichment.populate_sc_info", side_effect=lambda x: {**x, "sc_followers": 100})
    @patch("techno_scan.enrichment.get_cached_artist", return_value=None)
    ...
    @patch("techno_scan.enrichment.get_cached_artist")
    ...
    @patch("techno_scan.enrichment.populate_bandcamp_info", side_effect=lambda x: x)
    @patch("techno_scan.enrichment.populate_discogs_info", side_effect=lambda x: x)
    @patch("techno_scan.enrichment.populate_sc_info", side_effect=Exception("SC down"))
    @patch("techno_scan.enrichment.get_cached_artist", return_value=None)
...
class TestRunEnrichmentPhases:
    @patch("techno_scan.enrichment.store.batch_save_enriched")
    @patch("techno_scan.enrichment.check_rising")
    @patch("techno_scan.enrichment.populate_bandcamp_info", side_effect=lambda x: {**x, "bc_tags": "[]"})
    @patch("techno_scan.enrichment.populate_discogs_info", side_effect=lambda x: {**x, "dc_have": 10})
    @patch("techno_scan.enrichment.populate_sc_info", side_effect=lambda x: {**x, "sc_followers": 500})
    ...
    @patch("techno_scan.enrichment.populate_bandcamp_info", side_effect=lambda x: x)
    @patch("techno_scan.enrichment.populate_discogs_info", side_effect=lambda x: x)
    @patch("techno_scan.enrichment.populate_sc_info", side_effect=lambda x: x)
...
class TestEnrichBatchPhased:
    @patch("techno_scan.enrichment._run_enrichment_phases")
    @patch("techno_scan.enrichment.get_cached_artist")
    ...
    @patch("techno_scan.enrichment._run_enrichment_phases", return_value={"a1": {"name": "Enriched"}})
    @patch("techno_scan.enrichment.get_cached_artist", return_value=None)
...
class TestEnrichClubBatchPhased:
    @patch("techno_scan.enrichment._run_enrichment_phases", return_value={"club_1": {"name": "Club Artist"}})
    @patch("techno_scan.enrichment.search_sc_by_name", return_value="https://soundcloud.com/club-artist")
    @patch("techno_scan.enrichment.get_cached_artist", return_value=None)
